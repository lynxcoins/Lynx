                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nFeeNeeded < ::minRelayTxFee.GetFee(nBytes))
                {
                    ...
                    strFailReason = strprintf("Transaction too large for fee policy. \n Transaction size: %d, FeeNeeded: %d, FeeLimitation: %d and Number of input transactions: %d",
                    nBytes, nFeeNeeded, ::minRelayTxFee.GetFee(nBytes), txNew.vin.size());
                    return false;
                }
        ...
        if (GetTransactionWeight(*wtxNew.tx) >= MAX_STANDARD_TX_WEIGHT)
        {
            ...
            nBytes, GetTransactionWeight(*wtxNew.tx), MAX_STANDARD_TX_WEIGHT, txSize);
            return false;
        }
    }
